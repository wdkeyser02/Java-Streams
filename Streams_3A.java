...
public class Streams_3A {
    public static void main(String[] args) {
        ...
        List<Person_Result> result = people.stream()
            .map(person -> {
                Integer total_points = points.stream()
                    .<Integer>mapMulti((point, downstream ) -> {
                        if (point.personId.equals(person.id)) {
                            downstream.accept(point.points);
                        }
                    })
                    .reduce(0, (sum, point) -> sum + point);
                return new Person_Result(person.id, person.firstName, person.lastName, person.age, total_points);
            })
            .sorted((points1, points2) -> points2.total_points.compareTo(points1.total_points))
            .toList();
    ...
    private record Person_Result(Integer id, String firstName, String lastName, Integer age, Integer total_points) {}
}
