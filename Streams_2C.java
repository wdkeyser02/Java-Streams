...
public class Streams_2C {
    public static void main(String[] args) {
        ...
        List<Points_Result> points_Result = people.stream()
            .<Points_Result>mapMulti((person, downstream) -> person.points.stream()
                .forEach(point -> downstream.accept(new Points_Result(point.id, person.id, point.points, point.date)))
            )
            .toList();
